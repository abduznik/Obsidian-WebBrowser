...
class WebButtonModal extends Modal {
	onSubmit: (buttons: WebButton[], width: string, height: string, btnSize: string, defaultColor: string) => void;

	buttons: WebButton[] = [];
	containerEl!: HTMLElement;
	btnListEl!: HTMLElement;

	width: string = "100%";
	height: string = "500px";
	btnSize: string = "medium"; // small, medium, large
	defaultColor: string = "#007bff";

	constructor(app: App, onSubmit: (buttons: WebButton[], width: string, height: string, btnSize: string, defaultColor: string) => void) {
		super(app);
	...
	renderButtons() {
		...
		this.buttons.forEach((btn, idx) => {
			...
			labelInput.oninput = e => {
				this.buttons[idx].label = (e.target as HTMLInputElement).value;
			};
			...
			urlInput.oninput = e => {
				this.buttons[idx].url = (e.target as HTMLInputElement).value;
			};
			...
			colorInput.oninput = e => {
				this.buttons[idx].color = (e.target as HTMLInputElement).value;
			};
...
\`\`\``;
		...
		this.registerMarkdownCodeBlockProcessor("webblock", (source, el: HTMLElement) => {
			try {
				...
				const buttons: WebButton[] = JSON.parse(jsonStr);
